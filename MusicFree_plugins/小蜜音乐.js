...
async function getMediaSource(musicItem, quality) {
    // 0. 咪咕官方接口优先
    try {
        const mtm = await getMediaSourceByMTM(musicItem, quality);
        if (mtm && mtm.url && mtm.url.startsWith("http")) {
            return mtm;
        }
    } catch (e) {}
    ...
    for (const htLevel of [htLevelMap[quality] || "exhigh", "standard"]) {
        try {
            const res = (await axios_1.default.get("https://music.haitangw.cc/music/mg.php", {
